...
export class DirectAfalTransport implements AfalTransport {
  ...
  private async tryResolvePeerViaAgentCard(
    ...
    if (cardSignature) {
      ...
      const verifyContractOffers = parseSupportedContractOffers(params['supported_contract_offers']);
      if (verifyContractOffers) {
        verifyParams.supported_contract_offers = verifyContractOffers;
      }
